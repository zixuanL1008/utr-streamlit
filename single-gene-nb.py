...
parser.add_argument('-s', type=int, required=False ,default=1)
args = parser.parse_args()

DATA = '"D:\\UTR\\UTR-main\\data\\utrdb2.csv"'
BATCH_SIZE = args.bs
GENE = args.g
...
def select_best(scores, seqs, gc_control=False, GC_min=0.3, GC_max=0.7):
    ...
    参数
    ----
    scores : np.ndarray,  shape=(n_iter, n_batch)
    seqs   : list[list[str]], 与 scores 同形
    gc_control : bool, 是否启用过滤
    GC_min / GC_max : float, 0~1

    返回
    ----
    selected_seqs   : list[str], 长度 = n_batch
    ...
    for b in range(n_batch):
        ...
        candidates.sort(key=lambda x: -x[0])          # 高分在前
...
DIM = 40
SEQ_LEN = 128
gpath = 'D:\\UTR\\UTR-main\models\checkpoint_3000.h5'
exp_path = 'D:\\UTR\\UTR-main\models\humanMedian_trainepoch.11-0.426.h5'
tpath = 'D:\\UTR\\UTR-main\src\exp_optimization\script\checkpoint\RL_hard_share_MTL\3R\schedule_MTL-model_best_cv1.pth'
...
if OPTIMIZE:
    ...
    if GC_CONTROL:
        ...
        with open(f'D:\\UTR\\UTR-main\src\exp_optimization\outputs/{CELL_LINE}gc_init_exps_'+gene_name+'.txt', 'w') as f:
            for item in init_t:
                f.write(f'{item}\n')

        with open(f'D:\\UTR\\UTR-main\src\exp_optimization\outputs/{CELL_LINE}gc_opt_exps_'+gene_name+'.txt', 'w') as f:
            for item in best_scores:
                f.write(f'{item}\n')

        with open(f'D:\\UTR\\UTR-main\src\exp_optimization\outputs/{CELL_LINE}gc_best_seqs_'+gene_name+'.txt', 'w') as f:
            for item in best_seqs:
                f.write(f'{item}\n')

        with open(f'D:\\UTR\\UTR-main\src\exp_optimization\outputs/{CELL_LINE}gc_init_seqs_'+gene_name+'.txt', 'w') as f:
            for item in seqs_gen_init:
                f.write(f'{item}\n')

    else:
        with open(f'D:\\UTR\\UTR-main\src\exp_optimization\outputs/{CELL_LINE}init_exps_{gene_name}.txt', 'w') as f:
            for item in init_t:
                f.write(f'{item}\n')

        with open(f'D:\\UTR\\UTR-main\src\exp_optimization\outputs/{CELL_LINE}opt_exps_{gene_name}.txt', 'w') as f:
            for item in best_scores:
                f.write(f'{item}\n')

        with open(f'D:\\UTR\\UTR-main\src\exp_optimization\outputs/{CELL_LINE}best_seqs_{gene_name}.txt', 'w') as f:
            for item in best_seqs:
                f.write(f'{item}\n')

        with open(f'D:\\UTR\\UTR-main\src\exp_optimization\outputs/{CELL_LINE}init_seqs_{gene_name}.txt', 'w') as f:
            for item in seqs_gen_init:
                f.write(f'{item}\n')
    # ---------- 强制写出 bridge_run.py 需要的文件 ----------
    output_root = r"D:\UTR\UTR-main\src\exp_optimization\outputs"
    os.makedirs(output_root, exist_ok=True)

    with open(os.path.join(output_root, f"best_seqs_{gene_name}.txt"), 'w') as f:
        for seq in best_seqs:
            f.write(f"{seq}\n")

    with open(os.path.join(output_root, f"opt_exps_{gene_name}.txt"), 'w') as f:
        for score in best_scores:
            f.write(f"{score}\n")
    # 放在文件最后，print(...) 之前
    out_dir = r"D:\UTR\UTR-main\src\exp_optimization\outputs"
    os.makedirs(out_dir, exist_ok=True)

    # 统一文件名
    with open(os.path.join(out_dir, f"{gene_name}_best_seqs.txt"), "w") as f:
        for seq in best_seqs:
            f.write(f"{seq}\n")
    print(f"Results for {gene_name} saved to D:\\UTR\\UTR-main\src\exp_optimization\outputs")
